...
from transformers_pretraining.batching import BatchForLanguageModeling
from transformers_pretraining.bin.serve_dataset import add_batching_options
...
def query_batch(dataset_host: str) -> BatchForLanguageModeling:
    data = requests.get(dataset_host, stream=True).content
    return BatchForLanguageModeling.from_bytes(data)
...
def visualize_model_output(
    ...
    Args:
        batch (BatchForLanguageModeling): The input batch.
